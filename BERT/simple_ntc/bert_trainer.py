...
class EngineForBert(MyEngine):
    ...
    def train(engine, mini_batch):
        ...
        # Take feed-forward
        y_hat = engine.model(x, attention_mask=mask).logits # Hidden state just before entering softmax layer.
        # |y_hat| = (n, |c|) 
        # Representation for each class in the sample of mini-batch

        loss = engine.crit(y_hat, y) # Cross entropy loss --> logsoftmax(y_hat)
        loss.backward()
        ...
        p_norm = float(get_parameter_norm(engine.model.parameters())) # L2 norm of parameters
        g_norm = float(get_grad_norm(engine.model.parameters())) # L2 norm of gradients
    ...
    def validate(engine, mini_batch):
        ...
        with torch.no_grad(): # Since we don't need gradient calculations
            x, y = mini_batch['input_ids'], mini_batch['labels']
            x, y = x.to(engine.device), y.to(engine.device)
            mask = mini_batch['attention_mask']
            mask = mask.to(engine.device)
...
class BertTrainer(Trainer):
    ...
    def train(
        ...
        validation_engine.add_event_handler(
            Events.EPOCH_COMPLETED, # event
            EngineForBert.check_best, # function # Inherit from trainer.py
        )
